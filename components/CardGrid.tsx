...
/**
 * Сколько высоты добавляет лицо карточки: название в одну-две строки и цена.
 *
 * Замерено на настоящей карточке, а не взято на глаз: название в одну строку даёт
 * 60 пикселей, в две — 77, отсюда 68 посередине. После G4 лицо стало короче: вид,
 * дата, источник, заметки и ряд кнопок уехали на экран растения, и прежние 181
 * пиксель превратились в эти. Число участвует только в раскладке по колонкам —
 * ошибка в несколько пикселей меняет распределение, а не вид карточки.
 */
...
export default function CardGrid({ items }: { items: CardGridItem[] }) {
   ...
   *
   * До измерения рисуем две колонки: это тот же вид, что на телефоне, и он верен
   * для самого узкого экрана — меньше двух колонок теперь не бывает нигде.
   * Обратный выбор (сразу три) на телефоне дал бы заметный перескок раскладки на
   * первом кадре.
   *
  ...
  useEffect(() => {
    ...
    const measure = (width: number) => {
      if (width <= 0) return
      /*
       * Не меньше двух колонок — даже когда по ширине карточки влезает одна
       * (тикет G4).
       *
       * До этого правило было чистым «сколько влезет по 320 пикселей», и на
       * телефоне выходила одна колонка: коллекцию приходилось мотать по одной
       * карточке. Владелец выбрал две — на 375 это около 170 пикселей на карточку,
       * ровно поэтому с лица карточки и убрано всё, кроме фотографии, названия и
       * цены. Выше по ширине правило прежнее: на 768 — две, на 1232 — три.
       */
      const columns = Math.max(2, Math.floor((width + GAP) / (CARD_MIN_WIDTH + GAP)))
  ...
  const columnCount = layout?.columns ?? 2
