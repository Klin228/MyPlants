...
export interface Plant {
  ...
  /**
   * Цена — необязательное поле (тикет J5).
   *
   * До этого она была обязательной, и первое, что продукт делал с новым
   * человеком, — требовал назвать сумму. Барьер на самом хрупком шаге воронки, и
   * логическая странность: цена как раз то, что по умолчанию не показывается
   * никому.
   *
   * `undefined` и `0` — разные вещи, и их нельзя схлопывать: ноль это подарок или
   * черенок от подруги, то есть известная цена, а `undefined` — «не указал».
   * Сумма коллекции складывает только известные.
   */
  price?: number
  /**
